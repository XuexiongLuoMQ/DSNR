...
def node_classification(session, bs, seqne, sequences, seq_contents, seq_len, node_n, samp_idx, label, ratio):
    ...
    while e_idx < len(sequences):
        batch_enc = session.run(seqne.decoder_output, feed_dict={seqne.input_seqs: sequences[s_idx: e_idx],
                                                                 seqne.input_seq_content: seq_contents[s_idx: e_idx],
                                                                 seqne.dropout_rnn: 0., seqne.dropout_word: 0.,seqne.keep_prob:0.
                                                                 })
    ...
    if s_idx < len(sequences):
        batch_enc = session.run(seqne.decoder_output, feed_dict={seqne.input_seqs: sequences[s_idx: len(sequences)],
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        model = STNEConv(hidden_dim=h_dim, node_num=node_fea.shape[0], fea_dim=node_fea.shape[1],
                         ...
                         word_dim=word_dim, vocab_size=len(vocab), depth=dpt, attention_size=attention_size, filter_sizes=[1, 2, 3, 4, 5])
            ...
            if start_idx < len(node_seq):
                sess.run([train_op, model.loss_ce, model.global_step],
                         feed_dict={model.input_seqs: node_seq[start_idx:len(node_seq)],
                                    ...
                                    model.dropout_rnn: drop,model.keep_prob: keep_prob, model.dropout_word: word_drop})
